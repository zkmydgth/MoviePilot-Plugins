...
class TransferHandler:
    ...
    def __init__(self, client: P115Client, storage_name: str = "115网盘Plus"):
        ...
        self._jobview_cache: Dict[int, Any] = {}
    ...
    def _jobview_of(self, chain):
        """
        取得给定宿主链对象的 JobView 适配器（按对象缓存）。

        :param chain: 宿主 TransferChain 实例
        :return: JobViewAdapter
        """
        key = id(chain)
        adapter = self._jobview_cache.get(key)
        if adapter is None:
            adapter = get_jobview(chain)
            if adapter is None:
                # 宿主未提供队列时给出空实现，调用点无需逐处判空
                adapter = JobViewAdapter(None, owner="unavailable")
            self._jobview_cache[key] = adapter
        return adapter
    ...
    def _remove_completed_jobs(
        ...
        for (media_id, season), group_tasks in tasks_by_media.items():
            try:
                ...
                for task in group_tasks:
                    try:
                        ...
                        if task_action == "finish":
                            self._jobview_of(chain).finish_task(mp_task)
                        elif task_action == "fail":
                            self._jobview_of(chain).fail_task(mp_task)
                ...
                # 检查是否所有相关任务都完成了
                jobview = self._jobview_of(chain)
                check_func = getattr(jobview, check_method, None)
                if check_func and check_func(mp_sample_task):
                    # 移除整个媒体组任务
                    with self._task_lock():
                        jobview.remove_job(mp_sample_task)
                    removed_count += 1
            except Exception as e:
                logger.debug(
        ...
        for task in tasks:
            try:
                ...
                try:
                    chain = TransferChain()
                    mp_task = self._create_mp_task(task)
                    if self._jobview_of(chain).is_finished(mp_task):
                        with self._task_lock():
                            # 更新文件数量和大小
                            transferinfo.file_count = (
                                self._jobview_of(chain).count(
                                    task.mediainfo, task.meta.begin_season
                                )
                                or 1
                            )
                            transferinfo.total_size = (
                                self._jobview_of(chain).size(
                                    task.mediainfo, task.meta.begin_season
                                )
                            ...
                            if transferinfo.need_notify and (
                                ...
                                try:
                                    ...
                                    if task.mediainfo.type == MediaType.TV:
                                        season_episodes = self._jobview_of(chain).season_episodes(
                                            task.mediainfo,
                                            task.meta.begin_season,
                                        )
                                        if season_episodes:
    ...
    def _batch_delete_empty_dirs(self, tasks: List[TransferTask]) -> None:
        ...
        try:
            ...
            for (media_id, season), group_tasks in tasks_by_media.items():
                ...
                is_success = self._jobview_of(chain).is_success(mp_sample_task)
                logger.debug(
                ...
                if not is_success:
                    try:
                        # 任务状态诊断依赖宿主私有实现，取不到就跳过，不影响主流程
                        task_states = self._jobview_of(chain).describe_job_tasks(
                            sample_task.mediainfo,
                ...
                # 获取所有成功的任务
                success_tasks = self._jobview_of(chain).success_tasks(
