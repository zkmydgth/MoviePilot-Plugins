...
# 各补丁点依赖的宿主形参（不含 self），宿主删改任意一个都会导致补丁语义失效
_U115_PATCH_PARAMS: Dict[str, list] = {
...
# U115Pan 的期望归属模块，防止宿主把实现整体挪走后补丁打到别处
_U115_MODULE_HINT = "app.modules.filemanager.storages.u115"
...
class U115Patcher:
    ...
    @classmethod
    def _verify_target(cls, module: str) -> None:
        """
        校验单个补丁点在宿主上的存在性与签名。

        校验失败直接抛 ``PatchTargetError``：补丁打不上去应让调用方知道，
        而不是静默降级成"看起来生效但实际走原生实现"。
        """
        check_patch_target(
            U115Pan,
            module,
            expected_params=_U115_PATCH_PARAMS[module],
            module_hint=_U115_MODULE_HINT,
        )
    ...
    def enable(cls):
        ...
        if configer.get_config("upload_module_enhancement"):
            ...
            try:
                cls._verify_target("upload")
            except PatchTargetError as err:
                logger.error(f"【P115Open】上传接口补丁目标校验失败，跳过应用: {err}")
            else:
        ...
        if configer.transfer_module_enhancement and status:
            ...
            for module in modules:
                ...
                try:
                    cls._verify_target(module)
                except PatchTargetError as err:
                    logger.error(
                        f"【P115Open】{cls.log_map[module]}接口补丁目标校验失败，"
                        f"跳过应用: {err}"
                    )
                    continue
