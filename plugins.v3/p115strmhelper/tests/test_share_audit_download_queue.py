...
def _load_queue_module():
    ...
    schemas_module.MessageType = SimpleNamespace(Plugin="plugin")
    # 这里只覆盖 FileItem / MessageType 两个符号，插件仍会 ``from app.schemas.types
    # import MessageType``。若把 app.schemas 替换成没有 __path__ 的裸模块，子模块
    # 解析会失败（'app.schemas' is not a package）。因此保留包路径，让子模块导入
    # 继续走真实的宿主桩。
    _real_schemas = modules.get("app.schemas")
    schemas_module.__path__ = list(getattr(_real_schemas, "__path__", []))
