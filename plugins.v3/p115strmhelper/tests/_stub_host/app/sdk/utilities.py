...
* ``cut`` —— 中文分词入口（插件 ``db_manager/moviepilot_transfer.py`` 用它
  清洗历史记录路径），宿主由 jieba 提供
...
def cut(text: str, HMM: bool = False) -> List[str]:
    """宿主 ``app.sdk.utilities.cut`` 的等价替身（jieba 分词）。

    真实宿主转发到 jieba。这里优先用 jieba（CI 与本地均已声明依赖），
    缺失时退化为按空白切分，保证测试不因分词器缺失而失败。
    """
    if not text:
        return []
    try:
