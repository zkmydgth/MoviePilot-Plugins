...
try:
    # MoviePilot 构建期会在宿主根目录生成 version.py（内含 APP_VERSION）。
    from version import APP_VERSION
except ImportError:  # pragma: no cover - 取决于宿主部署形态
    try:
        # 兜底：部分部署形态把版本信息放在 app.runtime.version。
        from app.runtime.version import get_app_version as _get_app_version

        APP_VERSION = _get_app_version()
    except Exception:  # noqa: BLE001 - 版本号缺失不应阻断插件加载
        APP_VERSION = "unknown"
